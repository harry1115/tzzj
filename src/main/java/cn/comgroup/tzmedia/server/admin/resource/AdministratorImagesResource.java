...
import cn.comgroup.tzmedia.server.admin.entity.AdministratorImage;
...
/**
 * AdministratorImagesResource
 *
 * @author pcnsh197
 */
public class AdministratorImagesResource {

    private final Administrator administratorEntity; // appropriate jpa Administrator entity
    private final EntityManager em; // entity manager provided by parent resource
    private final ServletConfig servletConfig;
    private final String deployPath;

    public AdministratorImagesResource(EntityManager em, Administrator administratorEntity,
    ...
    public Response getImage(@QueryParam("pictureType") PictureType pictureType,
        ...
        AdministratorImage administratorImage = administratorEntity.getAdministratorImage(pictureType, imageName);
        if (administratorImage != null) {
            String fileSystemPath = administratorEntity.buildFileSystemPath(deployPath, 
                administratorImage.getImageName());
    ...
    public Response uploadPictures(@QueryParam("pictureType") PictureType pictureType,
        ...
        if (PictureType.MAIN.equals(pictureType) || PictureType.PORTRAIT.equals(pictureType)) {
            administratorEntity.removeImageAndDeleteOnFS(pictureType,
        ...
        String webDisplayPath = administratorEntity.buildWebDisplayPath(fileName);
        AdministratorImage administratorImage = new AdministratorImage(fileName,
                webDisplayPath, pictureType);
        administratorEntity.addImage(administratorImage);
        if (pictureType.equals(PictureType.SUBSIDIARY)) {
            String thumbnailFileName = FileUtil.generateCommonThumbnailFileName(fileName);
            String thumbnailFilePath = administratorEntity.buildFileSystemPath(deployPath, thumbnailFileName);
            FileUtil.writeThumbnail(fileSystemPath, thumbnailFilePath);
            AdministratorImage administratorThumbnailImage = new AdministratorImage(thumbnailFileName,
                    administratorEntity.buildWebDisplayPath(thumbnailFileName), PictureType.SUBTHUMB);
            administratorEntity.addImage(administratorThumbnailImage);
        }
        ...
        String output = "Administrator " + administratorEntity.getUserId()+ " picture "
    ...
    public void deleteImage(@QueryParam("imageName") String imageName) {
        PersistenceUtil.deleteImage(administratorEntity,PictureType.SUBSIDIARY,
